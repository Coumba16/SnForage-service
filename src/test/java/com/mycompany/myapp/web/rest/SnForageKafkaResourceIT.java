...
class SnForageKafkaResourceIT {
    ...
    void setup() {
        ...
        SnForageKafkaResource kafkaResource = new SnForageKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/sn-forage-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/sn-forage-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
